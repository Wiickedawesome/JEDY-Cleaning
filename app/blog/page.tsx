...
export default function BlogPage() {
  return (
    <main className="min-h-screen bg-cream-50">
      {/* Hero */}
      <section className="relative bg-gradient-to-br from-sage-900 via-sage-800 to-terracotta-700 text-cream-50 py-20 overflow-hidden">
            ...
            {posts.map((post) => (
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 text-sm text-sage-700 mb-4">
                    <span className="inline-block px-3 py-1 bg-white/70 rounded-full border border-sage-200 font-semibold tracking-wide">
                      {post.tag}
                    </span>
                  ...
                  <h2 className="text-2xl font-serif font-bold text-sage-900 mb-3">{post.title}</h2>
                  <p className="text-sage-700 mb-6 leading-relaxed">{post.excerpt}</p>
                  <Link
                    href={`/blog/${post.slug}`}
                    className="inline-block text-terracotta-600 font-semibold hover:text-terracotta-700 transition-colors"
                  >
                    Read more →
                  </Link>
                </div>
